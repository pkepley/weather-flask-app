...
def get_fcst_interp(airport_name):
    # Pull actual and forecast
    df_actl_times = get_actl(airport_name)
    df_fcst = get_fcst(airport_name)
...
def get_avf_compare(airport_name):
    df_actl = get_actl(airport_name)
    df_fcst_interp = get_fcst_interp(airport_name)
